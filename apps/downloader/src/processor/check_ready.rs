...
use tbh_database::download::{DownloadState, find_downloads_with_state, set_download_retries, set_download_state, set_download_lock};
use tbh_torbox::TorBoxApiState;
use tbh_torbox::usenet::list;

/// Checks all queued downloads for completion.
pub async fn process_ready_check(
...
async fn check_downloads(
    ...
    for download in find_downloads_with_state(&connection, DownloadState::Grabbing)? {
        ...
        if download_state.download_finished {
            set_download_retries(&connection, download.id, 0)?;
            set_download_state(&connection, download.id, DownloadState::Ready)?;
            info!(
                "Download {} has finished downloading on TorBox, preparing local download",
                &download.name
            );
            continue;
        }
        ...
        let in_progress = download_state.download_state == "downloading"
            || download_state.download_state == "processing";
        ...
        if download.retries > max_retries {
            ...
            error!(
                "Download {} exceeded maximum time limit, evicting download",
                &download.name
            );
            continue;
        }
        set_download_retries(&connection, download.id, download.retries + 1)?;
    }
    Ok(())
}
